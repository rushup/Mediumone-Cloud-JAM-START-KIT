...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    {
        printf("\r\n [E] MQTTConnect. MQTTSerialize_connect failed \r\n");
        goto exit;
    }    

    if ((rc = sendPacket(c, len, &connect_timer)) != OK)  // send the connect packet
    {  
        printf("\r\n [E] MQTTConnect. sendPacket failed \r\n");
        goto exit; // there was a problem
    } 
    ...
    if (waitfor(c, CONNACK, &connect_timer) == CONNACK)
    {
        ...
        {   
            printf("\r\n [E] MQTTConnect. MQTTDeserialize_connack failed.  \r\n");
            rc = FAILURE;
        }   
    }
    else
    {   
        printf("\r\n [E] MQTTConnect. Timeout expired on CONNACK.  \r\n");
        rc = FAILURE;
    }    
...
//    printf("\r\n");
    ...
    {
        printf("\r\n [E] MQTTSubscribe. Failed to send request. \r\n");        
        goto exit;             // there was a problem
    }    
    if (waitfor(c, SUBACK, &timer) == SUBACK)      // wait for suback 
    {
        ...
        { 
            printf("\r\n [E] MQTTSubscribe. Subscription request denied by broker. \r\n");        
            rc = FAILURE;
        }             
    }
    else 
    { 
        printf("\r\n [E] MQTTSubscribe. Timeout expired for suback reception \r\n");        
        rc = FAILURE;
    }   
